...
const UserCredentialsGrid = () => {
  ...
  if (credentials.length === 0) return <p>Nema dodatih kredencijala.</p>;
        ...
        {credentials.map((c) => (
          <div key={c.id} className="card credential-card">
            ...
            <div className="credential-body">
              <h3>{c.title}</h3>
              <p><strong>Veština:</strong> {c.skill.name} ({c.skill.category})</p>
              <p><strong>Tip:</strong> {c.type}</p>
              <p>
              ...
              {c.reviewed_by && (
                <p><strong>Odobrio:</strong> {c.reviewed_by.name}</p>
              )}
